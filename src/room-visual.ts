export function heatMap(room: Room, values: Array<Array<number>>) {
    ...
    const pad = (numStr: string) => numStr.length === 1 ? "0" + numStr : numStr;

    const normalToHex = (n: number) => "#"
        + pad(Math.round(255 * n).toString(16))
        + pad(Math.round(255 * (1 - n)).toString(16))
        + "00";

    const colors = _(normalized)
        .map((row: Array<number>) => _(row)
